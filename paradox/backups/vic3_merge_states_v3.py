...
def merge_states(states, merge_list_file):
    ...
    # Read the merge list from the file with utf-8-sig to handle BOM
    with open(merge_list_file, 'r', encoding='utf-8-sig') as f:
        merge_groups = [line.strip().split() for line in f.readlines()]
    
    # Merge states according to the merge list
    for group in merge_groups:
        # Strip state names to avoid issues with extra spaces or BOM
        group = [state.strip() for state in group]
        
        # Start merging all states in the group into the first state
        base_state = group[0]
        for state_name in group[1:]:
...
# output_file_path = "C:/Users/peng_/Documents/Paradox Interactive/Victoria 3/mod/tmp/t2/11_east_asia.txt"
...
# # Merge the states according to the merge list
# merged_states = merge_states(states, merge_list_file)
# write_results_to_file(merged_states, output_file_path)
